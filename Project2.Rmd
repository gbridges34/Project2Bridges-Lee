...
```{r}
set.seed(123)
# I might need to change this later
...
ggplot(train.data_by_day, aes(x=cnt, y=temp, color=season))+geom_point()+labs(x="Count of total rental bikes", y="Temperature", title="Scatterplot of Temperature vs Count of total rental bikes based off of seasons")
# +geom_smooth(aes(cnt,temp),method=lm), scale_color_gradient(low="blue", high="red")

# Scatterplot of Temperature vs Windspeed
ggplot(train.data_by_day, aes(x=temp,y=windspeed, color=season))+geom_point()+geom_smooth(aes(temp,windspeed),method=lm)+labs(x="Temperature", y="Windspeed", title="Scatterplot of Temperature vs Windspeed")
```
...
What Marcus will be doing tomorrow:

 explanation of ensemble model and a fit a boosted tree model
 going to have to convert my categorical variables and make sure I did the sampling for the training dataset correctly.

```{r}
# Did not include the categorical variables
model_1 <- lm(cnt ~ temp+atemp+hum+windspeed, data=train.data_by_day)
summary(model_1)
```
...
Going to have to convert my categorical variables.
