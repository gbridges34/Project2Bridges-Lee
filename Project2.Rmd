...
George decided to include weathersit, but exclude season. I will be using the 'cnt' variable in place of 'casual' or 'registered.' 
```{r}
data_days2 <- data_days %>% select(-c(instant, dteday, casual, registered, yr, mnth, holiday, workingday, weathersit))
```
```{r George data}
data_days22 <- data_days %>% select(-c(instant, dteday, casual, registered, yr, month, holiday, workingday, season))
```
...
From the summary statistics produced above, we can inspect the averages and standard deviations for the `temp`, `atemp` `hum`, `windspeed`, `cnt` variables grouped by  the `season` variable. Based on the sample size for each `season`, we can expect some of the variables to be skewed.
...
Looking at the boxplot, we can inspect whether the temperatures are skewed based off of season and see potential outliers.
